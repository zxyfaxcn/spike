...
class TcpChunkServer implements ChunkServerInterface
{
    ...
    public function handlePublicConnection(PublicConnection $publicConnection)
    {
        ...
        $requestProxyMessage = new Spike(Events::REQUEST_PROXY, $this->tunnel->toArray(), [
    ...
    public function setProxyConnection($publicConnectionId, ConnectionInterface $proxyConnection)
    {
        ...
        $publicConnection->pipe($proxyConnection);// 绑定用户请求链接与Proxy链接
        $proxyConnection->pipe($publicConnection->getConnection());// 绑定Proxy链接与用户请求链接
        $proxyConnection->write($publicConnection->getInitBuffer());// 转发用户请求内容至Proxy链接（Proxy链接有回应时回复用户请求）
